...
def get_save_dir(region, pagetype, domain):
    ...
    root_path = r"C:\hrithik\Research and Analysis Wing\requests\m_shein_com\HTMLs"
...
# Example Usage
region = "US"  # Replace this dynamically as per your use case
...
def make_request(document):
    ...
    """
    global success_count, failed_count, exception_count, main_list

    url = document['link']  # Assuming the document has a field 'url'
    # url = 'https://us.shein.com/Manfinity-Homme-Men-Solid-Collar-Long-Sleeve-Plain-Polo-Shirt-For-Work-p-21987945.html?mallCode=1&imgRatio=3-4'  # Assuming the document has a field 'url'
    ...
    try:
        ...
        # rating for us
        rating = json_data.get('info').get('comment').get('comments_overview').get('comment_rank_average')
        ...
        update_dict = {
            ...
            "rating": 0,
            "comments": '',  # Store the list of comment dictionaries
            "review_status_code": response.status_code
        }
        main_list.append(update_dict)
        exception_count += 1
...
df.to_excel('shein_us.xlsx', index=True)
